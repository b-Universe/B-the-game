module.exports = function registerSocialSockets(socket, io, state, deps) {
    ...
    socket.on('apartment_expand', (data) => {
      ...
      socket.emit('system_dialog', 'Apartment expanded successfully!');
    ...
    socket.on('apartment_unexpand', (data) => {
      ...
      io.emit('zones_config_data', state.zonesConfig);
      socket.emit('apartment_expanded');
      socket.emit('system_dialog', 'Apartment un-expanded. $5,000 refunded.');
      logSystem(`APARTMENT UN-EXPAND: ${player.name} refunded ${refund} for locking chunk ${chunkToLock} in ${myAptZone}`);
