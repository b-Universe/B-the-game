module.exports = function registerCombatSockets(socket, io, state, deps) {
    ...
    const { logSystem, randomUUID, getServerTerrainZ, checkLoS, handleNpcDeath } = deps;
    ...
    socket.on('combat_hit', (data) => {
      ...
      const attackerZ = attacker.z !== undefined ? attacker.z : getServerTerrainZ(attacker.x, attacker.y);
      const targetZ = target.z !== undefined ? target.z : getServerTerrainZ(target.x, target.y);

      const now = Date.now();
      if (power.isProjectile) { return; }
      else {
        ...
        if (!isDevPower) {
          ...
          if (!checkLoS(attacker.x, attacker.y, attackerZ, target.x, target.y, targetZ)) return;
        }
      }
      ...
      if (power.effects && power.effects.length > 0) {
        isCrit = Math.random() * 100 <= (power.stats?.critChance || 5);
        const critMult = isCrit ? (power.stats?.critMult || 1.5) : 1.0;

        power.effects.forEach(eff => {
          if (eff.type === 'Damage') {
            if (!isDeflect && Math.random() * 100 <= (eff.chance || 100)) dmg += (eff.magnitude || 0) * critMult;
          } else if (eff.type === 'Heal') {
            if (!isDeflect && Math.random() * 100 <= (eff.chance || 100)) {
              target.hp = Math.min(target.maxHp, target.hp + ((eff.magnitude || 0) * critMult));
              for (const npc of npcsCatalog) {
                  ...
                  if (npcHostileTo.includes(attackerGroup)) {
                      ...
                      if (distToTarget < 400 || distToHealer < 400) {
                          ...
                          npc.aggroList[socket.id] = (npc.aggroList[socket.id] || 0) + ((eff.magnitude || 0) * critMult) * 1.5;
                      }
                  }
              }
            }
            ...
            if (!isDeflect && Math.random() * 100 <= (eff.chance || 100)) {
              ...
              if (existing) { existing.endTime = Date.now() + ((eff.duration || 5.0) * 1000); existing.damage = (eff.magnitude || 0) * critMult; existing.tickRate = (eff.tickRate || 1.0) * 1000; }
              else { target.activeEffects.push({ id: `dot_${powerId}`, isDynamic: true, type: 'DoT', damage: (eff.magnitude || 0) * critMult, tickRate: (eff.tickRate || 1.0) * 1000, duration: (eff.duration || 5.0) * 1000, startTime: Date.now(), endTime: Date.now() + ((eff.duration || 5.0) * 1000), lastTick: Date.now() }); }
            }
            ...
            if (!isDeflect && Math.random() * 100 <= (eff.chance || 100)) {
              ...
              const resistance = Math.max(-1.0, Math.min(1.0, levelDiff * 0.15)); const durationMult = 1.0 - resistance;
        ...
        isCrit = Math.random() < (power.critChance || 0);
        dmg = (isCrit ? power.critDamage : power.damage) || 0;
      }
        ...
        if (targetType === 'npc') {
            ...
            if (!isDeflect && attacker) {
                ...
                if (expResult.expGained > 0) {
                    io.to(attacker.id).emit('chat_message', { type: 'system', name: 'System', text: `You gained ${expResult.expGained} EXP.` });
                    if (expResult.leveledUp) {
                        io.to(attacker.id).emit('chat_message', { type: 'system', name: 'System', text: `Congratulations! You reached Level ${expResult.newLevel}!` });
    ...
    socket.on('spawn_projectile', (data) => {
      ...
      if (power.effects && power.effects.length > 0) {
        isCrit = Math.random() * 100 <= ((power.stats?.critChance || 5) + (bonusCrit * 100)); const critMult = isCrit ? (power.stats?.critMult || 1.5) : 1.0;
        power.effects.forEach(eff => { if (eff.type === 'Damage' && Math.random() * 100 <= (eff.chance || 100)) { damage += (eff.magnitude || 0) * critMult; } });
      } else { isCrit = Math.random() < ((power.critChance || 0) + bonusCrit); damage = (isCrit ? power.critDamage : power.damage) || 0; }
      ...
      const proj = { uuid: randomUUID(), powerId: powerId, senderId: socket.id, senderName: attacker.name, startX: startX, startY: startY, startZ: startZ, x: startX, y: startY, z: startZ, targetX: startX + ((targetX - startX) / maxDist) * cappedDist, targetY: startY + ((targetY - startY) / maxDist) * cappedDist, targetZ: startZ + ((targetZ - startZ) / maxDist) * cappedDist, speed: power.visuals?.projectileSpeed !== undefined ? power.visuals.projectileSpeed : (power.projSpeed || 400), damage: damage, effects: power.effects || [], critMult: power.stats?.critMult || 1.5, isCrit: isCrit, maxDist: cappedDist, distTravelled: 0, active: true, zone: attacker.zone || 'untitled' };
      activeProjectiles.push(proj);
      io.to(attacker.zone || 'untitled').emit('spawn_projectile', { uuid: proj.uuid, powerId: proj.powerId, senderId: proj.senderId, startX: proj.startX, startY: proj.startY, startZ: proj.startZ, targetX: proj.targetX, targetY: proj.targetY, targetZ: proj.targetZ, speed: proj.speed, isCrit: proj.isCrit, damage: proj.damage });
    });
